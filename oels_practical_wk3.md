...
Each judgment trial involves showing the participant a sentence and getting a single keypress response from them, which we can achieve using the `html-keyboard-response` plugin from jsPsych. Details of the options for that plugin are in the [jsPsych documentation](https://www.jspsych.org/v8/plugins/html-keyboard-response/). We are using the `stimulus` parameter for the sentence the participant is judging, `prompt` reminds the participant what they are supposed to be doing, and `choices` shows the list of keyboard responses they are allowed to provide - in this case we only accept y or n keypresses (the `choices` parameter has a list of options enclosed in square brackets), so any other keypress is ignored. So the code for one judgment trial looks like this:
...
You will notice that the `stimulus` parameter here is quite complicated - it includes some HTML markup, including tags for headers (`<h3>` and `</h3>` to start and end a header), and paragraphs (`<p>` ... `</p>`). By default, jsPsych centers all text, which sometimes looks fine (e.g. for the judgment trials, where we *want* the stimulus to be centered) but I think it looks terrible for instruction text, so I also tell it to left-align that text and pad it a little from the left and right edge of the browser window, by adding some stuff inside the paragraph tags - I start a left-aligned and padded paragraph with `<p style='text-align:left; padding-left: 40px; padding-right: 40px'>`, then end it with `</p>`. Again, if you want to know what the text looks like without this additional formatting you can strip out all the `style=...` stuff and reload the experiment. Finally, the `choices` parameter for this trial type is a list of button labels - lists are enclosed in square brackets, here the list contains exactly one option with the "yes I consent" text, which produces a screen with exactly one button to be clicked. Finally, I have to use a backslash (`\`) character whenever I want to include a line break in the stimulus string, otherwise javascript thinks there is a syntax error. Those line breaks are just in there to make the stimulus text look nicer for people inspecting the code.
...
- [More challenging] How would you use [the instructions plugin](https://www.jspsych.org/v8/plugins/instructions/) to present the instructions?
